package com.quarkdata.yunpan.api.util.common.sensitive;
...
public class SensitiveWordUtil {
    ...
    private Logger logger = LoggerFactory.getLogger(SensitiveWordUtil.class);
    /**
     * 敏感词匹配规则
     */
    ...
    private void init() {
        ...
        try {
            ...
            BufferedReader br = new BufferedReader(new InputStreamReader(
                    new DefaultResourceLoader().getResource("sensi_words.txt").getInputStream(), StandardCharsets.UTF_8));
            while (br.readLine() != null) {
                if(StringUtils.isNotBlank(br.readLine())) {
                    sensitiveWordSet.add(br.readLine());
                }
            }
            initSensitiveWordMap(sensitiveWordSet);
            long endTime = System.currentTimeMillis();
            logger.info("========================初始化SensitiveWordUtil敏感词库完成, 耗时: {} ms=================", endTime - startTime);
        } catch (IOException e) {
            logger.error("+++++++++++++++初始化SensitiveWordUtil敏感词词库失败+++++++++++++++", e);
        }
    }
